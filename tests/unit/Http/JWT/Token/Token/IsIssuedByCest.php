...
class IsIssuedByCest
{
    /**
     * Unit Tests Phalcon\Http\JWT\Token\Token :: isIssuedBy()
     *
     * @since  2019-12-22
     */
    public function httpJWTTokenTokenIsIssuedBy(UnitTester $I)
    {
        $I->wantToTest('Http\JWT\Token\Token - isIssuedBy()');
        ...
        $I->assertFalse($token->isIssuedBy("abc"));

        $headers   = new Item(["typ" => "JWT"], "header-encoded");
        $claims    = new Item(["iss" => "issuer"], "claim-encoded");
        $signature = new Signature("signature-hash", "signature-encoded");

        $token = new Token($headers, $claims, $signature);

        $I->assertFalse($token->isIssuedBy("unknown"));
        ...
        $I->assertTrue($token->isIssuedBy("issuer"));
    }
}
